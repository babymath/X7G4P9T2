let userLanguage = null;
        ...
        startQuizButton.addEventListener('click', function () {
            ...
            const quizFileId = urlParams.get('quizFile');
        ...
        window.addEventListener('load', () => {
            ...
            const urlParams = new URLSearchParams(window.location.search);
            let quizFileId = urlParams.get('quizFile');

            if (!quizFileId || quizFileId.trim() === '') {
                console.warn('Quiz File ID is missing or invalid. Using a default quiz file ID.');
                quizFileId = 'defaultQuizFileId'; // Replace with a valid default file ID if available
            }

            // Validate the format of the quizFileId
            const fileIdRegex = /^[a-zA-Z0-9_-]{33}$/; // Google Drive file IDs are typically 33 characters long
            if (!fileIdRegex.test(quizFileId)) {
                console.error('Invalid Quiz File ID format.');
                alert('Invalid Quiz File ID format. Please check the URL and try again.');
                quizContentDiv.innerHTML = `<p style="color: red; text-align: center;">Invalid Quiz File ID format. <a href="index.html">Go back to selection</a></p>`;
                quizContentDiv.style.display = 'block';
                languageSelectionDiv.style.display = 'none';
                return;
            }
        ...
        function loadQuizData(quizFileId) {
            showLoadingSpinner(true); // Show spinner

            const fileUrl = `https://drive.google.com/uc?export=download&id=${quizFileId}`;
            console.log(`Attempting to download quiz file from URL: ${fileUrl}`); // Log the URL for debugging

            fetch(fileUrl)
                .then(response => {
                    if (response.status === 403) {
                        throw new Error("Access to the file is forbidden. Please ensure the file is publicly accessible or hosted on a CORS-enabled server.");
                    }
                    if (!response.ok) {
                        throw new Error(`HTTP error! status: ${response.status}, file ID: ${quizFileId}`);
                    }
                    return response.blob(); // Download the file as a blob
                })
                .then(blob => {
                    console.log('Quiz file downloaded successfully. Parsing content...');
                    return blob.text(); // Convert the blob to text
                })
                .then(content => {
                    try {
                        const data = JSON.parse(content); // Parse the JSON content
                        console.log('Quiz data successfully parsed:', data); // Log the parsed data
                        showLoadingSpinner(false); // Hide spinner
                        if (!Array.isArray(data) || data.length === 0) {
                            throw new Error("Quiz data is empty or not in the expected array format.");
                        }
                        questions = data;
                        ...
                        throw new Error("Invalid JSON format. The fetched content might not be a valid quiz file.");
                    }
                })
                .catch(error => {
                    showLoadingSpinner(false); // Hide spinner
                    console.error(`Error loading quiz data: ${error.message}`); // Log the error for debugging
                    alert(`Error loading quiz data. ${error.message} If the issue persists, consider downloading the file manually and hosting it on a CORS-enabled server.`);
                    quizContentDiv.innerHTML = `<p style="color: red; text-align: center;">Failed to load quiz questions. Please ensure the file is publicly accessible or hosted on a CORS-enabled server. <a href="index.html">Go back to selection</a></p>`;
                    quizContentDiv.style.display = 'block';
                    languageSelectionDiv.style.display = 'none';
                });
        }
        ...
        function showResults() {
            ...
            backToIndexButton.textContent = 'Back to Quiz Selection';
            backToIndexButton.addEventListener('click', () => {
                window.location.href = "index.html"; // Redirect to index.html
            });
            resultSectionDiv.appendChild(backToIndexButton);
